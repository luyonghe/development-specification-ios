...
## iOS 工程目录推荐标准
公司的项目多，为了方便移植和管理，iOS工程目录采用如下结构：
主目录按照功能模块分类，内目录按照业务分类（为了方便功能的移植，每个模块之间低耦合，相对独立）
优点：对模块的类集中化，方便管理与开发。
...
##5.	项目中的错误和警告：
###1.api太陈旧
项目中最好用最新的api。防止因为api太陈旧导致过期。

比如，在iOS6的一个api,放到iOS9就可能导致无法使用。

###2.项目中要尽量少的警告
在项目中，尽量少警告或者没有警告，警告是对当前项目中的一些做法不合理，系统认为有更好的方式。

###3.关于警告或者是错误的收集
我们项目有坑总结，遇到警告或者错误可以收集到对应的坑总结中。
...
分离控制器与模型、视图：不要把所有的 “业务逻辑”
放进跟视图有关的类中。这使代码非常难以复用。使用控制器类来处理这些代码，但保证控制器不需要了解太多表示层的逻辑。
